...
### 3.8 Board-backed projects — the issue is the work item

Some projects keep work items as files under `_todos/`. Others keep them on a tracker board. **On a
board-backed project the issue *is* the work item**, and every rule in this contract applies to it
unchanged. Without this section the contract describes a directory a board-backed project does not
have, and its required fields bind nothing — which is what happened on Coach Platform, where 30 of
35 backlog issues carried no milestone and no prerequisite because no rule named the field they go
in.

**A field goes in the board's own field wherever one exists.** A header line in the description is
the fallback, never the first choice: a real field can be queried, sorted and checked, and a line of
prose cannot.

| Contract field | Where it lives on a Paperclip board |
|---|---|
| The work item | The issue |
| `Id` | `identifier` (`WEE-143`) |
| `Status` | `status` — see the mapping below |
| `Milestone` | `goalId`, naming the **technical** milestone goal (`level: task`) |
| The product milestone | That goal's `parentId` (`level: company`). It is not set on the issue |
| `Blocked on: dependency · <id>` | `blockedBy`, naming the issue |
| `Blocked on: decision · <role>` | `unblockDescriptor.owner`, **and a pending card carrying the decision** |
| `Blocked on: external · <project>` | `unblockDescriptor`, naming the project |
| `Value` | `priority`: `high` · `medium` · `low`. `critical` is reserved for `Kind: defect` with `Value: high` |
| `Raised` | `createdAt` |
...
1. **`goalId`.** An issue with no goal is not schedulable: nothing will ever select it, and no
   ...
   in one line and the raiser states who decides where it goes.
...
issue to a goal has until now only covered the parent issue opened when a milestone starts.

**Ranking is §3.5 unchanged.** It is a query over these fields once they are filled, and a reading
exercise while they are not.
